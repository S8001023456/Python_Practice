iden_dic = {'A': 10 ,'B': 11, 'C': 12, 'D': 13,'E': 14,'F': 15,'G': 16,'H': 17,'I': 34,'J': 18,'K': 19,'L': 20,'M': 21,
            'N': 22 ,'O': 35, 'P': 23, 'Q': 24,'R': 25,'S': 26,'T': 27,'U': 28,'V': 29,'W': 32,'X': 30,'Y': 31,'Z': 33}
iden_str = input()
iden_engnum = iden_dic[iden_str[0]] % 10 * 9 + iden_dic[iden_str[0]] // 10
other_num_sum = 0
for digits in range(8, 0, -1):
    other_num_sum += int(iden_str[digits]) * (9 - digits)
all_num_sum = iden_engnum + other_num_sum + int(iden_str[9])
if all_num_sum % 10 == 0:
    print('real')
else:
    print('fake')
